    ...
    public static class CompilerRegistry
    {
        public sealed class RegistryImpl : IScriptCompilerRegistry
        {
            ...
            private IScriptAssembly Compile(UGUI user, Dictionary<int, string> shbangs, UUID assetID, TextReader reader, int linenumber = 1, CultureInfo cultureInfo = null, Func<string, TextReader> includeOpen = null)
            {
                ...
                {
                    return compiler.Compile(AppDomain.CurrentDomain, user, shbangs, assetID, reader, linenumber, cultureInfo, includeOpen);
                }
                else
                {
                    ...
                    try
                    {
                        var assembly = compiler.Compile(appDom, user, shbangs, assetID, reader, linenumber, cultureInfo, includeOpen);
            ...
            private void SyntaxCheckAndDump(Stream s, UGUI user, Dictionary<int, string> shbangs, UUID assetID, TextReader reader, int linenumber = 1, CultureInfo cultureInfo = null, Func<string, TextReader> includeOpen = null)
            {
                ...
                compiler.SyntaxCheckAndDump(s, user, shbangs, assetID, reader, linenumber, cultureInfo, includeOpen);
            }
            ...
            public void SyntaxCheck(UGUI user, UUID assetID, TextReader reader, CultureInfo cultureInfo = null, Func<string, TextReader> includeOpen = null)
            {
                ...
                using (var headReader = new StreamReaderAddHead(header.ToString(), reader))
                {
                    Compile(user, shbangs, assetID, headReader, cultureInfo: cultureInfo, includeOpen: includeOpen);
                }
            }
