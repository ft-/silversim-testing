...
namespace SilverSim.Scene.Agent.Bakery.SubBakers.Clothing
{
    public sealed class ShoesSubBaker : AbstractSubBaker
    {
        ...
        private readonly double m_ShoeHeight;
        private readonly double m_ShoeHeightBump;
        ...
        private static Color3 GetShoeColor(Wearable shoe)
        {
            ...
            double val;
            if (shoe.Params.TryGetValue(812, out val))
            {
                col.R = val.LimitRange(0, 1);
            }
            if (shoe.Params.TryGetValue(813, out val))
            {
                col.G = val.LimitRange(0, 1);
            }
            if (shoe.Params.TryGetValue(817, out val))
            {
