...
{
    public sealed partial class ExperienceBrokerService : IExperiencePermissionsInterface
    {
        Dictionary<UEI, bool> IExperiencePermissionsInterface.this[UGUI agent] =>
            GetExperienceService(agent).Permissions[agent];
        ...
        bool IExperiencePermissionsInterface.Remove(UEI experienceID, UGUI agent)
        {
            ...
            return TryGetExperienceService(experienceID, out experienceService) && experienceService.Permissions.Remove(experienceID, agent);
        }
