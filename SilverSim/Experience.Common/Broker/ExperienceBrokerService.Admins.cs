...
{
    public sealed partial class ExperienceBrokerService : IExperienceAdminInterface
    {
        List<UEI> IExperienceAdminInterface.this[UGUI agent] =>
            GetExperienceService(agent).Admins[agent];
        ...
        bool IExperienceAdminInterface.TryGetValue(UEI experienceID, UGUI agent, out bool allowed)
        {
            ...
            allowed = false;
            return TryGetExperienceService(experienceID, out experienceService) && experienceService.Admins.TryGetValue(experienceID, agent, out allowed);
        }
    }
}
