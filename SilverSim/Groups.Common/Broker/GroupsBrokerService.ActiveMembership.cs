...
{
    public sealed partial class GroupsBrokerService : IActiveGroupMembershipInterface
    {
        bool IActiveGroupMembershipInterface.ContainsKey(UGUI requestingAgent, UGUI principal)
        {
            GroupsServiceInterface groupsService;
            return TryGetGroupsService(principal, out groupsService) && groupsService.ActiveMembership.ContainsKey(requestingAgent, principal); 
        }
