...
{
    [UDPMessage(MessageType.InitiateDownload)]
    [Reliable]
    [Trusted]
    public class InitiateDownload : Message
    {
        public UUID AgentID;
        ...
        public static Message Decode(UDPPacket p) => new InitiateDownload()
        {
            ...
            ViewerFilename = p.ReadStringLen8()
        };

        public override void Serialize(UDPPacket p)
        {
            p.WriteUUID(AgentID);
            p.WriteStringLen8(SimFilename);
            p.WriteStringLen8(ViewerFilename);
        }
    }
}
